...
// Helper function to redirect to login with current path
...
export const errorInterceptor = async (error) => {
	...
	if (SkipAuthHeader.includes(originalRequest.url))
		return Promise.reject(error);

	// If no refresh token, destroy session and redirect to login
