...
export default function PopUpModal({
  ...
  useEffect(() => {
    ...
    let stand = standards.filter(
      (standard) => standard.id === standardIdForUpdate,
    )[0];
    setSelectedSection(
      sections.filter((section) => section.id === stand.section.id)[0],
    );
    ...
    if (stand.custom_days.length > 0) {
      ...
      setDays(tmpDays);
      setCurrentDays(
        tmpDays.filter((tmpDay) => stand.custom_days.includes(tmpDay.id)),
      );
    }
  }, [standardIdForUpdate]);
    ...
    updateStandard(
      ...
      (res) => {
        handleSuccessExec(res, data, t("success-edit-standard-msg"), () => {
          setStandards(
            standards.map((stand) =>
              stand.id !== standardIdForUpdate ? stand : data,
            ),
          );
        });
      },
  ...
  return ReactDom.createPortal(
    <>
      ...
      <Container position={position}>
        ...
        <DropDown value={templateType} onChange={handleFormTypeChange}>
          <Option disabled selected hidden value="">
            {t("choose-type")}
          </Option>
          <Option value="NumberPointTemplate">{t("digital")}</Option>
          <Option value="CheckboxPointTemplate">{t("true-false")}</Option>
          <Option value="PointTemplate">{t("text-needs-revision")}</Option>
        </DropDown>
        ...
        <AddButton onClick={handleAddStandardSubmit}>
          {standardIdForUpdate ? t("edit-criteria") : t("add-criteria")}
        </AddButton>
      </Container>
    </>,
