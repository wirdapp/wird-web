...
function getTableProps(studentData, t) {
  const dataArray = studentData.map((element, index) => ({
...
function TableData({ selectedUser, selectedDay }) {
  ...
  const dispatch = (action) => {
    changeTableProps((prevState) => {
      ...
      if (!isNaN(action.row) && action.data) {
        let selectedPoint = tableProps.data.filter(
          (point) => point.pointID === action.data.pointID,
        )[0];
        if (selectedPoint) {
      ...
      if (action.columnKey === "point") {
        if (tempPrevArr)
          setEditedOrRemovedPointData(
            getDifference(tempPrevArr, prevState.data)[0],
          );
      }
  ...
  const updateCells = () => {
    if (kaPropsUtils.isValid(tableProps)) {
      if (editedOrRemovedPointData?.pointID) {
        updateStudentPoint(
          selectedUser,
          editedOrRemovedPointData?.pointID,
          {
    ...
    if (selectedDay !== "" && selectedUser !== "") {
      retrieveStudentsPointsPerDay(
        ...
        (res) => {
          ...
          if (res.data.student_points)
            changeTableProps(getTableProps(res.data.student_points, t));
  ...
  if (selectedUser === "" && selectedDay === "") {
    return (
      <div className="table-msg-text-section">
        <H5>{t("selectStudentAndDay")}</H5>
      </div>
    );
  } else if (selectedUser === "") {
    return (
      <div className="table-msg-text-section">
        <H5>{t("selectStudent")} </H5>
      </div>
    );
  } else if (selectedDay === "") {
    return (
