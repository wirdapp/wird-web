...
export default function EditStandardForm(props) {
    ...
    updateStandard(
      ...
      (res) => {
        if (res && res.status === 200) {
          let updatedStandard = props.standards.filter(
            (standard) => standard.id === Number(selectedStandard.id),
          )[0];
  ...
  const handleSelectedStandardChange = (e) => {
    if (e.target.value === "") {
      ...
      let standard = props.standards.filter(
        (standard) => standard.id === e.target.value,
      )[0];
      setSelectedStandard(standard);
      setSelectedSection(
        props.sections.filter(
          (section) => section.id === standard.section.id,
        )[0],
      );
      ...
      if (standard.custom_days.length > 0) {
        ...
        setCurrentSelectedDays(
          days.filter((day) => selectedArray.includes(day.id + "")),
        );
      } else {
        setCustomDaysChecked(false);
        setCustomDays([]);
    ...
    <Formm onSubmit={handleEditStandardSubmit}>
      {props.standards && props.standards.length > 0 ? (
        <DropdownDiv>
          <DropdownListStanderd
            ...
            onChange={handleSelectedStandardChange}
            value={
              Object.keys(selectedStandard).length > 0
                ? selectedStandard.id
                : ""
            }
          >
            ...
            {props.standards.map((standard, index) => {
              return (
                <DropdownListItemStanderd key={index + 1} value={standard.id}>
                  {standard.label}
                </DropdownListItemStanderd>
              );
            })}
      ...
      {props.sections && props.sections.length > 0 ? (
        <DropdownDiv>
          <DropdownListStanderd
            className="DropdownList"
            value={
              Object.keys(selectedSection).length > 0 ? selectedSection.id : ""
            }
      ...
      {formType !== "chk" && (
        <>
          ...
          <DivTxtFieldnumber>
            <Span />
            <FormInputnumber
              min={lowerUnitsBound !== -1 ? lowerUnitsBound : 0 + ""}
              type="number"
              value={upperUnitsBound !== -1 ? upperUnitsBound + "" : ""}
      ...
      <DivTxtFieldnumber>
        ...
        <FormInputnumber
          type="number"
          min="1"
          value={pointsPerUnit !== -1 ? pointsPerUnit + "" : ""}
